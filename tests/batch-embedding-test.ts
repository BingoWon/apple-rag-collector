...
async function runBatchEmbeddingTest(): Promise<void> {
  ...
  try {
    ...
    console.log(`⚡ Average Time per Text: ${Math.round(duration / testTexts.length)}ms`);
    ...
    console.log(`✅ Consistent dimensions: ${embeddings.every(e => e && e.length === (embeddings[0]?.length || 0))}`);
    console.log(`✅ No empty embeddings: ${embeddings.every(e => e && e.length > 0)}`);
...
async function testEmptyArray(): Promise<void> {
