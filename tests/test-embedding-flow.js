...
function testEmbeddingFlow() {
  ...
  const chunkFile = 'tests/batch-chunker-comparison-output/2025-08-29T00-24-58Z/url_1/typescript_chunks.json';
  ...
  try {
    ...
    embeddingTexts.forEach((text, index) => {
      ...
      const hasTitle = lines[0] && !lines[0].startsWith('#') && !lines[0].startsWith('##');
    ...
    console.log('    - Contains JSON syntax:', oldMethod.includes('{"title":') ? 'Yes' : 'No');
    
    console.log('  New method (title + content):');
    console.log('    - Length:', newMethod.length);
    console.log('    - Contains JSON syntax:', newMethod.includes('{"title":') ? 'Yes' : 'No');
    console.log('    - Starts with title:', !newMethod.startsWith('#') ? 'Yes' : 'No');
    console.log('');
    ...
    console.log('💾 Sample saved to: tests/embedding-flow-test-output/embedding-flow-sample.json');
