...
async function collectContent(url) {
  try {
    ...
    console.log(
      `API Result for ${url}:`,
      apiResult[0]?.data ? "success" : "failed",
      apiResult[0]?.error
    );

    if (!apiResult[0]?.data) {
      throw new Error(
        `Failed to fetch document: ${apiResult[0]?.error || "Unknown error"}`
      );
    }

    const processedResult = await contentProcessor.processDocuments(apiResult);
    console.log(
      `Processed Result for ${url}:`,
      processedResult[0]?.data ? "success" : "failed",
      processedResult[0]?.error
    );
    console.log(
      `Processed Result structure:`,
      JSON.stringify(processedResult[0], null, 2)
    );

    if (!processedResult[0]?.data) {
      throw new Error(
        `Failed to process document: ${processedResult[0]?.error || "Unknown error"}`
      );
    }
...
function compareChunks(tsChunks, pyChunks, url) {
  ...
  if (tsChunks.length !== pyChunks.length) {
    comparison.differences.push(
      `Chunk count mismatch: TS=${tsChunks.length}, Python=${pyChunks.length}`
    );
  }
  ...
  for (let i = 0; i < maxChunks; i++) {
    ...
    if (tsChunk === pyChunk) {
      ...
      comparison.differences.push(
        `Chunk ${i} differs (TS: ${tsChunk.length} chars, Python: ${pyChunk.length} chars)`
      );
    }
  }

  comparison.identical =
    identicalChunks === maxChunks && tsChunks.length === pyChunks.length;
...
async function testSingleUrl(url, index, total) {
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(urlDir, "processed_result.json"),
      JSON.stringify(processedData, null, 2)
    );
    fs.writeFileSync(
      path.join(urlDir, "typescript_chunks.json"),
      JSON.stringify(tsChunks, null, 2)
    );
    fs.writeFileSync(
      path.join(urlDir, "python_chunks.json"),
      JSON.stringify(pyChunks, null, 2)
    );
    fs.writeFileSync(
      path.join(urlDir, "comparison.json"),
    ...
    if (comparison.identical) {
      ...
      logError(`URL ${index + 1}/${total} FAILED: Results differ`, {
...
async function runBatchTest() {
  ...
  try {
    ...
    // Save summary report
    fs.writeFileSync(
      path.join(SESSION_DIR, "batch_test_summary.json"),
    ...
    console.log(
      `Success Rate: ${((summary.passedUrls / summary.totalUrls) * 100).toFixed(1)}%`
    );
    ...
    if (summary.failedUrls === 0) {
      logSuccess(
        "All tests passed! Python and TypeScript implementations are identical across all URLs."
      );
      return true;
    } else {
      logError(
        `${summary.failedUrls} out of ${summary.totalUrls} tests failed.`
      );
      return false;
    }
  } catch (error) {
